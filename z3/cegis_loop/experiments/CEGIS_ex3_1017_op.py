...
spec:
Node parse_gre:
packet.extract(hdr.gre);  --> field0 bit<32>
transition select(hdr.gre.C, hdr.gre.R, hdr.gre.K, hdr.gre.S, hdr.gre.s, 
hdr.gre.recurse, hdr.gre.flags, hdr.gre.ver, hdr.gre.proto) { --> [1][1][1][1][1][3][5][3][16]
...
Node parse_nvgre:
packet.extract(hdr.nvgre);  --> field1 bit<32>
transition accept;

Node parse_erspan_t3:
packet.extract(hdr.erspan_t3_header); --> field2 bit<96>
transition accept;
"""

# List the parser's information
input_bit_stream_size = 32 + 96
pkt_field_size_list = [32, 32, 96]
num_pkt_fields = len(pkt_field_size_list)
tran_key_candidate = [[i for i in range(32)],[],[],[],[]]
...
def specification(Input_bitstream, initial_field_val_list):
    ...
    O_field1 = If(O_field0 == BitVecVal(0b00100000000000000110010101011000, 32), 
    ...
    O_field2 = If(O_field0 == BitVecVal(0b00000000000000000010001011101011, 32), 
...
def spec(Input_bitstream, initial_list):
    ...
    if Fields[0] == "0010000000000000" + "0110010101011000":
    ...
    elif Fields[0] == "0000000000000000" + "0010001011101011":
