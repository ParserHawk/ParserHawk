    ...
    for i in range(len(alloc_matrix)):
        for j in range(len(alloc_matrix[i])):
            if key_sel == None:
                ...
                key_sel = If(alloc_matrix[i][j] == node_id, Concat(key_sel, Extract(tran_key_candidate[i][j],tran_key_candidate[i][j],update_field_val_l[i])), Concat(dummy, key_sel))    
