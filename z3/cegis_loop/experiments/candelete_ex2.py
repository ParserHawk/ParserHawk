...
"""
spec:
Node parse_fabric_header:
packet.extract(hdr.fabric_header); --> field0 bit<40>
transition select(hdr.fabric_header.packetType) {  -->field[0:2]
...
Node parse_fabric_header_unicast:
packet.extract(hdr.fabric_header_unicast);  --> field1 bit<24>
transition accept;

Node parse_fabric_header_multicast:
packet.extract(hdr.fabric_header_multicast); --> field2 bit<24>
transition accept;

Node parse_fabric_header_mirror:
packet.extract(hdr.fabric_header_mirror); --> field3 bit<56>
transition accept;

Node parse_fabric_header_cpu:
packet.extract(hdr.fabric_header_cpu); --> field4 bit<88>
transition accept;
"""

# List the parser's information
input_bit_stream_size = 40 + 88
pkt_field_size_list = [40, 24, 24, 56, 88]
num_pkt_fields = len(pkt_field_size_list)
tran_key_candidate = [[39,38,37],[],[],[],[]]
...
def specification(Input_bitstream, initial_field_val_list):
    ...
    O_field0 = Extract(input_bit_stream_size - 1, input_bit_stream_size - pkt_field_size_list[0], Input_bitstream) #node 0
    O_field1 = If(Extract(pkt_field_size_list[0] - 1, pkt_field_size_list[0] - 3, O_field0) == BitVecVal(0b001, 3), 
                  Extract(input_bit_stream_size - pkt_field_size_list[0] - 1, input_bit_stream_size - pkt_field_size_list[0] - pkt_field_size_list[1], Input_bitstream), initial_field_val_list[1])
    O_field2 = If(Extract(pkt_field_size_list[0] - 1, pkt_field_size_list[0] - 3, O_field0) == BitVecVal(0b010, 3), 
                  Extract(input_bit_stream_size - pkt_field_size_list[0] - 1, input_bit_stream_size - pkt_field_size_list[0] - pkt_field_size_list[2], Input_bitstream), initial_field_val_list[2])
    O_field3 = If(Extract(pkt_field_size_list[0] - 1, pkt_field_size_list[0] - 3, O_field0) == BitVecVal(0b011, 3), 
                  Extract(input_bit_stream_size - pkt_field_size_list[0] - 1, input_bit_stream_size - pkt_field_size_list[0] - pkt_field_size_list[3], Input_bitstream), initial_field_val_list[3])
    O_field4 = If(Extract(pkt_field_size_list[0] - 1, pkt_field_size_list[0] - 3, O_field0) == BitVecVal(0b101, 3), 
                  Extract(input_bit_stream_size - pkt_field_size_list[0] - 1, input_bit_stream_size - pkt_field_size_list[0] - pkt_field_size_list[4], Input_bitstream), initial_field_val_list[4])
...
def generate_key_expr_list(pos, I, Dist, F):
    ret_l = []
    for i in range(len(pkt_field_size_list)):
        ret_l.append(dynamic_extract_loop(pos, I, Dist, F[i], pkt_field_size_list[i], field_id=i))
    return ret_l
    ...
    for i in range(len(alloc_matrix)):
        ...
        for j in range(len(alloc_matrix[i])):
            if key_sel == None:
                ...
                # tran_key_candidate
                key_sel = If(alloc_matrix[i][j] == node_id, Extract(tran_key_candidate[i][j],tran_key_candidate[i][j],update_field_val_l[i]),dummy)    
            else:
                # key_sel = If(alloc_matrix[i][j] == node_id, Concat(key_sel, Extract(j,j,update_field_val_l[i])), 
                #              Concat(dummy, key_sel))    
                key_sel = If(alloc_matrix[i][j] == node_id, Concat(key_sel, Extract(tran_key_candidate[i][j],tran_key_candidate[i][j],update_field_val_l[i])), 
                             Concat(dummy, key_sel))    
...
def node0(Dist, F, I, idx, pos, alloc_matrix, Lookahead, key_val_list, tran_idx_list, default_idx_node, extract_status, s):
    nodeID = 0
    key_expr_list = generate_key_expr_list(pos, I, Dist, F)
...
def node4(Dist, F, I, idx, pos, alloc_matrix, Lookahead, key_val_list, tran_idx_list, default_idx_node, extract_status, s):
    nodeID = 4
    key_expr_list = generate_key_expr_list(pos, I, Dist, F)
    update_field_val_l = generate_update_field_val(idx, Dist, F, key_expr_list, alloc_matrix, node_id = nodeID)
    post_pos = post_node_pos(idx = idx, Dist = Dist, node_id = nodeID, alloc_matrix=alloc_matrix, pos = pos)
...
def node5(Dist, F, I, idx, pos, alloc_matrix, Lookahead, key_val_list, tran_idx_list, default_idx_node, extract_status, s):
    nodeID = 5
    key_expr_list = generate_key_expr_list(pos, I, Dist, F)
    update_field_val_l = generate_update_field_val(idx, Dist, F, key_expr_list, alloc_matrix, node_id = nodeID)
    post_pos = post_node_pos(idx = idx, Dist = Dist, node_id = nodeID, alloc_matrix=alloc_matrix, pos = pos)
...
def temporary_bitvec_for_counterexample(I_val, random_initial_value_list, num_pkt_fields, testcaseID):
    ...
    input_fields = []
    for i in range(num_pkt_fields):
        input_field = BitVec(f'input_field{i}_{testcaseID}', pkt_field_size_list[i])
    ...
    return Input_bitstream, input_fields, extract_status, constraint
...
def alloc_matrix_gen(pkt_field_size_list):
    ...
    for i in range(len(pkt_field_size_list)):  
        row = []  
        for j in range(len(tran_key_candidate[i])):
...
def synthesis_step(cexamples):
    ...
    print("alloc_matrix =", alloc_matrix)
    for i in range(3, len(alloc_matrix[0])):
        s.add(alloc_matrix[0][i] == -1)
    # s.add(alloc_matrix[0][0] == 0)
    # s.add(alloc_matrix[0][1] == 0)
    # s.add(alloc_matrix[0][2] == 0)
    ...
    if not cexamples:
        ...
        for j in range(len(cexamples)):
            ...
            spec_out = spec(format(Input_bitval, f'0{input_bit_stream_size}b'), random_initial_value_list)
...
def main():
    ...
    # Define expected arguments with "=" style
    parser.add_argument('--lookahead_window_size', type=int, required=True, help='Size of the lookahead window')
    parser.add_argument('--size_of_key', type=int, required=True, help='Size of the key')
    parser.add_argument('--num_transitions', type=int, required=True, help='Number of transitions')
    parser.add_argument('--num_parser_nodes', type=int, required=True, help='Number of parser nodes')
    parser.add_argument('--flag', type=bool, required=True, help='Simplify the transition key selection')

    # Parse the arguments
    args = parser.parse_args()

    # List the hardware configuration
    global lookahead_window_size
    global size_of_key
    global num_transitions
    global num_parser_nodes
    global flag

    lookahead_window_size = args.lookahead_window_size
    size_of_key = args.size_of_key
    num_transitions = args.num_transitions
    num_parser_nodes = args.num_parser_nodes
    flag = args.flag
    cegis_loop()
