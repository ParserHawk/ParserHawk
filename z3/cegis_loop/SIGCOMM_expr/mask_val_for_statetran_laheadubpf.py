...
spec:
Node0:
if (lookahead[0:7] == 0b1) {
    goto node1;
} else if (lookahead[0:7] == 0b10){
    goto node2;
} else if (lookahead[0:7] == 0b100){
    goto node3;
} else {
    exit;
}
...
input_bit_stream_size = 8

pkt_field_size_list = [1, 1, 1]
num_pkt_fields = len(pkt_field_size_list)

# List the hardware configuration
lookahead_window_size = 8
size_of_key = 8
num_parser_nodes = 4
...
def specification(Input_bitstream, initial_field_val_list):
    ...
    #  pos 0   1  2  3 4
    #  idx 13 12 11 10 9 8 
    O_field0 = If(Extract(input_bit_stream_size - 1, input_bit_stream_size - 1 - 8 + 1, Input_bitstream) == BitVecVal(0b00000001, 8), Extract(input_bit_stream_size - 1, input_bit_stream_size - 1 - pkt_field_size_list[0] + 1, Input_bitstream), initial_field_val_list[0])
    O_field1 = If(Extract(input_bit_stream_size - 1, input_bit_stream_size - 1 - 8 + 1, Input_bitstream) == BitVecVal(0b00000010, 8), Extract(input_bit_stream_size - 1, input_bit_stream_size - 1 - pkt_field_size_list[0] + 1, Input_bitstream), initial_field_val_list[1])
    O_field2 = If(Extract(input_bit_stream_size - 1, input_bit_stream_size - 1 - 8 + 1, Input_bitstream) == BitVecVal(0b00000100, 8), Extract(input_bit_stream_size - 1, input_bit_stream_size - 1 - pkt_field_size_list[0] + 1, Input_bitstream), initial_field_val_list[2])
...
def spec(Input_bitstream, initial_list):
    ...
    if Input_bitstream[0: 8] == "00000001":
        Fields[0] = Input_bitstream[0 : pkt_field_size_list[0]]
    elif Input_bitstream[0: 8] == "00000010":
        Fields[1] = Input_bitstream[0 : pkt_field_size_list[1]]
    elif Input_bitstream[0: 8] == "00000100":
        Fields[2] = Input_bitstream[0 : pkt_field_size_list[2]]
    l = []
...
def implementation(Flags, Input_bitstream, idx, pos, random_initial_value_list, 
    ...
    # Out_Fields_0, post_pos_0, idx_0, post_extract_status_0 = nodes[0](Flags[0], Out_Fields, Input_bitstream, 
    #                                                                     idx=idx, pos=post_pos, alloc_matrix=alloc_matrix, 
    #                                                                     Lookahead=Lookahead, 
    #                                                                     key_val_list=key_val_2D_list[0], 
    #                                                                     tran_idx_list=tran_idx_2D_list[0], 
    ...
    # Out_Fields_1, post_pos_1, idx_1, post_extract_status_1 = nodes[1](Flags[1], Out_Fields, Input_bitstream, 
    #                                                                     idx=idx, pos=post_pos, alloc_matrix=alloc_matrix, 
    #                                                                     Lookahead=Lookahead, 
    #                                                                     key_val_list=key_val_2D_list[1], 
    #                                                                     tran_idx_list=tran_idx_2D_list[1], 
    #                                                                     default_idx_node=default_idx_node_list[1], 
    #                                                                     extract_status=post_extract_status, s=s)
    # Out_Fields_2, post_pos_2, idx_2, post_extract_status_2 = nodes[2](Flags[2], Out_Fields, Input_bitstream, 
    #                                                                     idx=idx, pos=post_pos, alloc_matrix=alloc_matrix, 
    #                                                                     Lookahead=Lookahead, 
    #                                                                     key_val_list=key_val_2D_list[2], 
    #                                                                     tran_idx_list=tran_idx_2D_list[2], 
    #                                                                     default_idx_node=default_idx_node_list[2], 
    #                                                                     extract_status=post_extract_status, s=s)
    # Out_Fields_3, post_pos_3, idx_3, post_extract_status_3 = nodes[3](Flags[3], Out_Fields, Input_bitstream, 
    #                                                                     idx=idx, pos=post_pos, alloc_matrix=alloc_matrix, 
    #                                                                     Lookahead=Lookahead, 
    #                                                                     key_val_list=key_val_2D_list[3], 
    #                                                                     tran_idx_list=tran_idx_2D_list[3], 
    #                                                                     default_idx_node=default_idx_node_list[3], 
    #                                                                     extract_status=post_extract_status, s=s)
    # Out_Fields = [If(condition0, then_ele, else_ele) for then_ele, else_ele in zip(Out_Fields_0, Out_Fields)]
    # Out_Fields = [If(condition1, then_ele, else_ele) for then_ele, else_ele in zip(Out_Fields_1, Out_Fields)]
    # Out_Fields = [If(condition2, then_ele, else_ele) for then_ele, else_ele in zip(Out_Fields_2, Out_Fields)]
    # Out_Fields = [If(condition3, then_ele, else_ele) for then_ele, else_ele in zip(Out_Fields_3, Out_Fields)]
    # post_pos = If(condition0, post_pos_0, post_pos)
    # post_pos = If(condition1, post_pos_1, post_pos)
    # post_pos = If(condition2, post_pos_2, post_pos)
    # post_pos = If(condition3, post_pos_3, post_pos)
    # idx = If(condition0, idx_0, idx)
    # idx = If(condition1, idx_1, idx)
    # idx = If(condition2, idx_2, idx)
    # idx = If(condition3, idx_3, idx)
    # post_extract_status = [If(condition0, then_ele, else_ele) for then_ele, else_ele in zip(post_extract_status_0, post_extract_status)]
    # post_extract_status = [If(condition1, then_ele, else_ele) for then_ele, else_ele in zip(post_extract_status_1, post_extract_status)]
    # post_extract_status = [If(condition2, then_ele, else_ele) for then_ele, else_ele in zip(post_extract_status_2, post_extract_status)]
    # post_extract_status = [If(condition3, then_ele, else_ele) for then_ele, else_ele in zip(post_extract_status_3, post_extract_status)]
    # # -------------
    # condition0 = idx == 0
    # condition1 = idx == 1
    # condition2 = idx == 2
