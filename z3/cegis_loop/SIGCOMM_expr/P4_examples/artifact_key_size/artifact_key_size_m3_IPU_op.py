...
def specification(Input_bitstream, initial_field_val_list):
    ...
    Or_condition = Or(Extract(1, 0, O_field0) == BitVecVal(0b00, 2), Extract(1, 0, O_field0) == BitVecVal(0b01, 2))
    O_field1 = If(And(Extract(32, 2, O_field0) == BitVecVal(0x0000000, 31), Or_condition), Extract(input_bit_stream_size - 1 - pkt_field_size_list[0], input_bit_stream_size - 1 - pkt_field_size_list[0] - pkt_field_size_list[1] + 1, Input_bitstream), initial_field_val_list[1])
...
def spec(Input_bitstream, initial_list):
    ...
    if int(Fields[0][0 : 31], 2) == 0 and (int(Fields[0][31 : 33], 2) == 1 or int(Fields[0][31 : 33], 2) == 0):
