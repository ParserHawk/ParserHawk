...
input_bit_stream_size = 8+8+8

pkt_field_size_list = [8, 8, 8]
...
def specification(Input_bitstream, initial_field_val_list):
    ...
    l.append(O_field1)
    O_field2 = If(And((Extract(7, 0, O_field0) == BitVecVal(0b0, 8), Extract(7, 0, O_field1) == BitVecVal(0b0, 8))), Extract(input_bit_stream_size - 1 - pkt_field_size_list[0] - pkt_field_size_list[1], input_bit_stream_size - 1 - pkt_field_size_list[0] - pkt_field_size_list[1] - pkt_field_size_list[2] + 1, Input_bitstream), initial_field_val_list[2])
    l.append(O_field2)
...
def spec(Input_bitstream, initial_list):
    ...
    if Fields[0][0 : 8] == "00000000" and Fields[1][0 : 8] == "00000000":
...
def temporary_bitvec_for_counterexample(I_val, random_initial_value_list, num_pkt_fields, testcaseID):
    ...
    return Input_bitstream, field_l, extract_status, constraint
