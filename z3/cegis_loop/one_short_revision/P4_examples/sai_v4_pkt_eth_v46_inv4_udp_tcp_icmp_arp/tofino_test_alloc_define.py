...
state parse_packet_out_header {
    packet.extract(headers.packet_out_header);  // 1-bit field0
    transition parse_ethernet;
  }

  state parse_ethernet {
    packet.extract(headers.ethernet);   // 16-bit field1
    transition select(headers.ethernet.ether_type) {
      ETHERTYPE_IPV4: parse_ipv4; // 0x0800 field2
      ETHERTYPE_IPV6: parse_ipv6; // 0x86dd field3
      ETHERTYPE_ARP:  parse_arp;  // 0x0806 field8
      _:              accept;
    }
  }

  state parse_ipv4 {  
    packet.extract(headers.ipv4); // 8-bit field2
    transition select(headers.ipv4.protocol) {
      IP_PROTOCOL_IPV4: parse_ipv4_in_ip; // 0x04 field4
  ...
  state parse_ipv6 {
    packet.extract(headers.ipv6);  // 8-bit field3
  ...
  state parse_ipv4_in_ip {
    packet.extract(headers.inner_ipv4);  // 8-bit field4
    transition select(headers.inner_ipv4.protocol) {
      IP_PROTOCOL_UDP:  parse_udp; // 0x11 field5
      IP_PROTOCOL_ICMP: parse_icmp; // 0x01 field7
      IP_PROTOCOL_TCP:  parse_tcp; // 0x06 field6
      _:                accept;
    }
  }

  state parse_udp {
    packet.extract(headers.udp);  // 1-bit field5
    transition accept;
  }

  state parse_tcp {
    packet.extract(headers.tcp);  // 1-bit field6
    transition accept;
  }

  state parse_icmp {
    packet.extract(headers.icmp); // 1-bit field7
    transition accept;
  }

  state parse_arp {
    packet.extract(headers.arp);  // 1-bit field8
    transition accept;
  }
...
input_bit_stream_size = 1 + 16 + 8 + 8 + 1

pkt_field_size_list = [1, 16, 8, 8, 8, 1, 1, 1, 1]
key_field_list = [0, 16, 8, 8, 8, 0, 0, 0, 0]
num_pkt_fields = len(pkt_field_size_list)
time_to_visit_tcam_tbl = 4
...
def specification(Input_bitstream, initial_field_val_list):
    ...
    O_field8 = If(Cond3, Extract(pos_beg, pos_beg  - pkt_field_size_list[8] + 1, Input_bitstream), initial_field_val_list[8])
    ...
    Cond7 = Extract(7, 0, O_field3) == BitVecVal(0x11, 8)
    Cond8 = Extract(7, 0, O_field3) == BitVecVal(0x06, 8)
    Cond9 = Extract(7, 0, O_field3) == BitVecVal(0x3a, 8)

    field4_cond = Extract(7, 0, O_field2) == BitVecVal(0x04, 8)
    O_field4 = If(And(Cond1, field4_cond), Extract(pos_beg  - pkt_field_size_list[2], pos_beg  - pkt_field_size_list[2] - pkt_field_size_list[4] + 1, Input_bitstream), initial_field_val_list[4])
...
#     }
#   }
    field5_cond = Extract(7, 0, O_field4) == BitVecVal(0x11, 8)
    field6_cond = Extract(7, 0, O_field4) == BitVecVal(0x06, 8)
    field7_cond = Extract(7, 0, O_field4) == BitVecVal(0x01, 8)

    O_field5 = If(And(Cond1, Cond4), Extract(pos_beg  - pkt_field_size_list[2], pos_beg  - pkt_field_size_list[2] - pkt_field_size_list[5] + 1, Input_bitstream), initial_field_val_list[5])
    O_field5 = If(And(Cond2, Cond7), Extract(pos_beg  - pkt_field_size_list[2], pos_beg  - pkt_field_size_list[2] - pkt_field_size_list[5] + 1, Input_bitstream), O_field5)
    O_field5 = If(And(Cond1, field4_cond, field5_cond), Extract(pos_beg  - pkt_field_size_list[2] - pkt_field_size_list[4], pos_beg  - pkt_field_size_list[2] - pkt_field_size_list[4] - pkt_field_size_list[5] + 1, Input_bitstream), O_field5)

    O_field6 = If(And(Cond1, Cond5), Extract(pos_beg  - pkt_field_size_list[2], pos_beg  - pkt_field_size_list[2] - pkt_field_size_list[6] + 1, Input_bitstream), initial_field_val_list[6])
    O_field6 = If(And(Cond2, Cond8), Extract(pos_beg  - pkt_field_size_list[2], pos_beg  - pkt_field_size_list[2] - pkt_field_size_list[6] + 1, Input_bitstream), O_field6)
    O_field6 = If(And(Cond1, field4_cond, field6_cond), Extract(pos_beg  - pkt_field_size_list[2] - pkt_field_size_list[4], pos_beg  - pkt_field_size_list[2] - pkt_field_size_list[4] - pkt_field_size_list[6] + 1, Input_bitstream), O_field6)

    O_field7 = If(And(Cond1, Cond6), Extract(pos_beg  - pkt_field_size_list[2], pos_beg  - pkt_field_size_list[2] - pkt_field_size_list[7] + 1, Input_bitstream), initial_field_val_list[7])
    O_field7 = If(And(Cond2, Cond9), Extract(pos_beg  - pkt_field_size_list[2], pos_beg  - pkt_field_size_list[2] - pkt_field_size_list[7] + 1, Input_bitstream), O_field7)
    O_field7 = If(And(Cond1, field4_cond, field7_cond), Extract(pos_beg  - pkt_field_size_list[2] - pkt_field_size_list[4], pos_beg  - pkt_field_size_list[2] - pkt_field_size_list[4] - pkt_field_size_list[7] + 1, Input_bitstream), O_field7)

    return [O_field0, O_field1, O_field2, O_field3, O_field4, O_field5, O_field6, O_field7, O_field8]
...
def spec(Input_bitstream, initial_list):
    ...
    if int(Fields[1][0 : 16], 2) == 0x0800:
        ...
        if int(Fields[2][0 : 8], 2) == 0x04:
            Fields[4] = Input_bitstream[curr_pos + pkt_field_size_list[2] : curr_pos + pkt_field_size_list[2] + pkt_field_size_list[4]]
            new_pos = curr_pos + pkt_field_size_list[2] + pkt_field_size_list[4]
            if int(Fields[4][0 : 8], 2) == 0x11:
                Fields[5] = Input_bitstream[new_pos : new_pos + pkt_field_size_list[5]]
            elif int(Fields[4][0 : 8], 2) == 0x01:    
                Fields[7] = Input_bitstream[new_pos : new_pos + pkt_field_size_list[7]]
            elif int(Fields[4][0 : 8], 2) == 0x06:
                Fields[6] = Input_bitstream[new_pos : new_pos + pkt_field_size_list[6]]
        elif int(Fields[2][0 : 8], 2) == 0x11:
            Fields[5] = Input_bitstream[curr_pos + pkt_field_size_list[2] : curr_pos + pkt_field_size_list[2] + pkt_field_size_list[5]]
        elif int(Fields[2][0 : 8], 2) == 0x06:
            Fields[6] = Input_bitstream[curr_pos + pkt_field_size_list[2] : curr_pos + pkt_field_size_list[2] + pkt_field_size_list[6]]
        elif int(Fields[2][0 : 8], 2) == 0x01:
            Fields[7] = Input_bitstream[curr_pos + pkt_field_size_list[2] : curr_pos + pkt_field_size_list[2] + pkt_field_size_list[7]]
    elif int(Fields[1][0 : 16], 2) == 0x86dd:
        Fields[3] = Input_bitstream[curr_pos : curr_pos + pkt_field_size_list[3]]
        if int(Fields[3][0 : 8], 2) == 0x11:
            Fields[5] = Input_bitstream[curr_pos + pkt_field_size_list[3] : curr_pos + pkt_field_size_list[3] + pkt_field_size_list[5]]
        elif int(Fields[3][0 : 8], 2) == 0x06:
            Fields[6] = Input_bitstream[curr_pos + pkt_field_size_list[3] : curr_pos + pkt_field_size_list[3] + pkt_field_size_list[6]]
        elif int(Fields[3][0 : 8], 2) == 0x3a:
            Fields[7] = Input_bitstream[curr_pos + pkt_field_size_list[3] : curr_pos + pkt_field_size_list[3] + pkt_field_size_list[7]]
    elif int(Fields[1][0 : 16], 2) == 0x0806:
        Fields[8] = Input_bitstream[curr_pos : curr_pos + pkt_field_size_list[8]]
    l = []
...
def synthesis_step(cexamples):
    ...
    for i in range(len(alloc_matrix)):
        for j in range(len(alloc_matrix[i])):
            for k in range(j + 1, len(alloc_matrix[i])):
                s.add(Or(alloc_matrix[i][j] == alloc_matrix[i][k], alloc_matrix[i][j] == -1, alloc_matrix[i][k] == -1))
            
            # s.add(Or(alloc_matrix[i][j] ==  alloc_idx_list[i], alloc_matrix[i][j] == -1))
    ...
    for i in range(tcam_num):
        ...
        s.add(Or(key_val_total_list[i] == 0x0800, key_val_total_list[i] == 0x86dd, key_val_total_list[i] == 0x0806, 
                 key_val_total_list[i] == 0x01, key_val_total_list[i] == 0x06, key_val_total_list[i] == 0x11,
                 key_val_total_list[i] == 0x3a, key_val_total_list[i] == 0x04))  # Add more values as needed
    # Update search space
    if not has_run:
