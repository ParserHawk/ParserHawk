    ...
    for i in range(1, len(parser_node_pipe)):
        for j in range(tcam_num):
            ...
            s.add(Or(key_val_total_list[i][j] == 0x0800, key_val_total_list[i][j] == 0x86dd, key_val_total_list[i][j] == 0x0806, 
                 key_val_total_list[i][j] == 0x01, key_val_total_list[i][j] == 0x06, key_val_total_list[i][j] == 0x11,
                 key_val_total_list[i][j] == 0x3a, key_val_total_list[i][j] == 0x04, 
                 And(key_val_total_list[i][j] >= 0, key_val_total_list[i][j] <= 0x0FFF)))  # Add more values as needed
    if not has_run:
