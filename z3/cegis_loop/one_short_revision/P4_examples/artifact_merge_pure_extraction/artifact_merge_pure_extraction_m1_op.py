    ...
    for j in range(lookahead_window_size):
        for i in range(input_bit_stream_size):
            if input_bit_stream_size - 1 - i - j < 0:
                break
            if key_sel == None:
                key_sel = If(And(Lookahead[node_id][j] == 1, post_node_pos == i), Extract(input_bit_stream_size - 1 - i - j, input_bit_stream_size - 1 - i - j, I), dummy)
            else:
...
def temporary_bitvec_for_counterexample(I_val, random_initial_value_list, num_pkt_fields, testcaseID):
    ...
    return Input_bitstream, [input_field0, input_field1, input_field2, input_field3, input_field4], constraint
