...
def implementation(Flags, Input_bitstream, idx, pos, random_initial_value_list, 
    ...
    for i in range(num_parser_nodes):
        if i == 0:
            ...
            assign_id = 0
            curr_sum = 0
            for j in range(num_parser_nodes):
...
def synthesis_step(cexamples):
    ...
    #         s.add(Or(key_val_total_list[i][j] == 0x876d, key_val_total_list[i][j] == 1, key_val_total_list[i][j] == 2, key_val_total_list[i][j] == 3))
