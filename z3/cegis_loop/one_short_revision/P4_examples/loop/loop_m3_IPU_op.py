...
def synthesis_step(cexamples):
    ...
    for v in parser_node_pipe:
        if len(sum_l) == 0:
            ...
            sum_l.append(sum_l[-1] + v)
    # Add constraints to the state transition idx, transit to parser nodes in future stages
    ...
    # print("default_idx_node_list =", default_idx_node_list)
    ID = 0
    for i in range(len(parser_node_pipe)):
    ...
    for i in range(len(parser_node_pipe)):
        for j in range(tcam_num):
            s.add(assignments[i][j] >= 0)
            if j >= 1:
                s.add(assignments[i][j - 1] <= assignments[i][j])
    constraints = []
    for i in range(len(parser_node_pipe)):
        for j in range(tcam_num):
            if i == 0:
                ...
                constraints.append(Or(And(assignments[i][j] < sum_l[i], assignments[i][j] >= sum_l[i - 1]), assignments[i][j] > num_parser_nodes))
    s.add(constraints)
    for i in range(len(alloc_matrix)):
    ...
    for i in range(1, len(parser_node_pipe)):
        for j in range(tcam_num):
            s.add(key_mask_total_list[i][j] == 0xF)
    for i in range(len(parser_node_pipe)):
        for j in range(tcam_num):
            s.add(Or(key_val_total_list[i][j] == 0, key_val_total_list[i][j] == 1, key_val_total_list[i][j] == 4, key_val_total_list[i][j] == 6))
...
def verification_step(model, cexamples):
    ...
    constraints = []
    for i in range(num_pkt_fields): 
        constraints.append(O_Impl[i] != O_Spec[i])
