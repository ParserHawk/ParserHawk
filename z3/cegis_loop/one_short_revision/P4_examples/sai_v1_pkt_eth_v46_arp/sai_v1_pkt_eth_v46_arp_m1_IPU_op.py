...
def synthesis_step(cexamples):
    ...
    for i in range(1, len(parser_node_pipe)):
        for j in range(tcam_num):
            ...
            s.add(Or(key_val_total_list[i][j] == 0x0800, key_val_total_list[i][j] == 0x86dd, 
                     key_val_total_list[i][j] == 0x0806, And(key_val_total_list[i][j] >= 0x0, key_val_total_list[i][j] <= 0xFF)))  # Add more values as needed
    if not has_run:
