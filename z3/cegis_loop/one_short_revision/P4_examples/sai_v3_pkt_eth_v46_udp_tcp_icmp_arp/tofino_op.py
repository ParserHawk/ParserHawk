...
state parse_packet_out_header {
  ...
  state parse_udp {
    packet.extract(headers.udp);  // 1-bit field4
    transition accept;
  }

  state parse_tcp { 
    packet.extract(headers.tcp);  // 1-bit field5
    transition accept;
  }

  state parse_icmp {
    packet.extract(headers.icmp); // 1-bit field6
    transition accept;
  }

  state parse_arp {
    packet.extract(headers.arp);  // 1-bit field7
    transition accept;
  }
"""
...
pkt_field_size_list = [1, 16, 8, 8, 1, 1, 1, 1]
key_field_list = [0, 16, 8, 8, 0, 0, 0, 0]
...
def specification(Input_bitstream, initial_field_val_list):
    ...
    O_field7 = If(Cond3, Extract(pos_beg, pos_beg  - pkt_field_size_list[4] + 1, Input_bitstream), initial_field_val_list[7])
    ...
    O_field4 = If(And(Cond2, Cond7), Extract(pos_beg  - pkt_field_size_list[2], pos_beg  - pkt_field_size_list[2] - pkt_field_size_list[4] + 1, Input_bitstream), O_field4)
    ...
    O_field5 = If(And(Cond2, Cond8), Extract(pos_beg  - pkt_field_size_list[2], pos_beg  - pkt_field_size_list[2] - pkt_field_size_list[5] + 1, Input_bitstream), O_field5)
    ...
    O_field6 = If(And(Cond2, Cond9), Extract(pos_beg  - pkt_field_size_list[2], pos_beg  - pkt_field_size_list[2] - pkt_field_size_list[6] + 1, Input_bitstream), O_field6)
...
def spec(Input_bitstream, initial_list):
    ...
    if int(Fields[1][0 : 16], 2) == 0x0800:
        ...
        if int(Fields[3][0 : 8], 2) == 0x11:
            Fields[4] = Input_bitstream[curr_pos + pkt_field_size_list[3] : curr_pos + pkt_field_size_list[3] + pkt_field_size_list[4]]
        ...
        Fields[7] = Input_bitstream[curr_pos : curr_pos + pkt_field_size_list[7]]
    l = []
...
def synthesis_step(cexamples):
    ...
    for i in range(tcam_num):
        ...
        s.add(Or(key_val_total_list[i] == 0x0800, key_val_total_list[i] == 0x86dd, key_val_total_list[i] == 0x0806, 
                 ...
                 key_val_total_list[i] == 0x3a))  # Add more values as needed
    # Update search space
    if not has_run:
