...
input_bit_stream_size = 160+320

pkt_field_size_list = [160, 320]
key_field_list = [160, 320]
...
def specification(Input_bitstream, initial_field_val_list):
    ...
    Cond_field1 = Extract(4 - 1, 0, O_field0) != BitVecVal(0x5, 4)
...
def synthesis_step(cexamples):
    ...
    for i in range(len(parser_node_pipe)):
        for j in range(tcam_num):
            s.add(key_val_total_list[i][j] == 5)
