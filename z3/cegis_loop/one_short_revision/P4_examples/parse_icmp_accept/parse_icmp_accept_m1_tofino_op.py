...
def specification(Input_bitstream, initial_field_val_list):
    ...
    O_field1 = If(And(Extract(15, 12, O_field0) == BitVecVal(0x8, 4), (Extract(11, 8, O_field0) & BitVecVal(0xe, 4)) != BitVecVal(0x0, 4)), Extract(input_bit_stream_size - 1 - pkt_field_size_list[0], input_bit_stream_size - 1 - pkt_field_size_list[0] - pkt_field_size_list[1] + 1, Input_bitstream), initial_field_val_list[1])
...
def spec(Input_bitstream, initial_list):
    ...
    if (Fields[0][0 : 4] == "1000") and ((int(Fields[0][4 : 8], 2) & int("1110", 2)) != int("0000", 2)):
...
def synthesis_step(cexamples):
    ...
    #     s.add(Or(key_val_total_list[i] == 0x8200, key_val_total_list[i] == 0x8400, key_val_total_list[i] == 0x8800))
    s.add(Flags[0][0] == 1)
    s.add(Flags[1][1] == 1)
    if not has_run:
