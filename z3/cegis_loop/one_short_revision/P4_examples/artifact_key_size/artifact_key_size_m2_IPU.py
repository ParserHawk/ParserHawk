...
def specification(Input_bitstream, initial_field_val_list):
    ...
    Or_condition = Or(Extract(7, 7, O_field0) == BitVecVal(0b0, 1), Extract(7, 7, O_field0) == BitVecVal(0b1, 1))
    O_field1 = If(And(Extract(39, 8, O_field0) == BitVecVal(0x00000000, 32), Or_condition), Extract(input_bit_stream_size - 1 - pkt_field_size_list[0], input_bit_stream_size - 1 - pkt_field_size_list[0] - pkt_field_size_list[1] + 1, Input_bitstream), initial_field_val_list[1])
...
def spec(Input_bitstream, initial_list):
    ...
    if int(Fields[0][8 : 8 + 32], 2) == 0 and (Fields[0][8+32 : 8+33] == '1' or Fields[0][8+32 : 8+33] == '0'):
