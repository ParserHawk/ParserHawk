...
Node1:
extract(hdr.data); // bit<8> data; --> field1

Node2:
accept;
"""

input_bit_stream_size = 48+24

pkt_field_size_list = [48, 24]
...
def specification(Input_bitstream, initial_field_val_list):
    ...
    O_field1 = If((Extract(39, 7, O_field0) == BitVecVal(0x00000000, 33)), Extract(input_bit_stream_size - 1 - pkt_field_size_list[0], input_bit_stream_size - 1 - pkt_field_size_list[0] - pkt_field_size_list[1] + 1, Input_bitstream), initial_field_val_list[1])
    O_field1 = If((Extract(39, 7, O_field0) == BitVecVal(0x00000001, 33)), Extract(input_bit_stream_size - 1 - pkt_field_size_list[0], input_bit_stream_size - 1 - pkt_field_size_list[0] - pkt_field_size_list[1] + 1, Input_bitstream), O_field1)
...
def spec(Input_bitstream, initial_list):
    ...
    if int(Fields[0][8 : 8 + 33], 2) == 0 or int(Fields[0][8 : 8 + 33], 2) == 1:
...
def node0(Dist, F, I, idx, pos, alloc_matrix, Lookahead, assignments, key_val_total_list, key_mask_total_list, tran_idx_total_list, default_idx_node, s):
    nodeID = 0
    # for i in range(num_pkt_fields):
