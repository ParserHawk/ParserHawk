...
def specification(Input_bitstream, initial_field_val_list):
    ...
    O_field1 = If(And((Extract(pkt_field_size_list[0] - 1, pkt_field_size_list[0] - 1 -7, O_field0) == BitVecVal(0b00000000, 8), Extract(pkt_field_size_list[0] - 1 - 8, pkt_field_size_list[0] - 1 - 8 - 3, O_field0) == BitVecVal(0b0000, 4))), Extract(input_bit_stream_size - 1 - pkt_field_size_list[0], input_bit_stream_size - 1 - pkt_field_size_list[0] - pkt_field_size_list[1] + 1, Input_bitstream), initial_field_val_list[1])
    O_field1 = If(And((Extract(pkt_field_size_list[0] - 1, pkt_field_size_list[0] - 1 -7, O_field0) == BitVecVal(0b00000000, 8), Extract(pkt_field_size_list[0] - 1 - 8, pkt_field_size_list[0] - 1 - 8 - 3, O_field0) == BitVecVal(0b0001, 4))), Extract(input_bit_stream_size - 1 - pkt_field_size_list[0], input_bit_stream_size - 1 - pkt_field_size_list[0] - pkt_field_size_list[1] + 1, Input_bitstream), O_field1)
...
def synthesis_step(cexamples):
    ...
    for i in range(1, len(parser_node_pipe)):
        for j in range(tcam_num):
            s.add(Or(key_val_total_list[i][j] == 0x0, key_val_total_list[i][j] == 0x1))
    if not has_run:
