...
def codegen(json_obj, number_of_parser_nodes, size_of_key):
    ...
    for v in sorted_keys:
        ...
        # assign_stage_{j}_tcam{i}
        # key_val_stage_{j}_tcam{i}
        # key_mask_stage_{j}_tcam{i}
        # tran_idx_stage_{j}_tcam{i}
        # default_idx_node_{i}
        elif v[0:16] == "default_idx_node":
            pattern = r"default_idx_node_(\d+)"
            match = re.search(pattern, v)
            if match:
                nodeID = int(match.group(1))
                node_list[nodeID]["default_tran"] = data[v]
        elif v[0:8] == "tran_idx":
            pattern = r"tran_idx_stage_(\d+)_tcam(\d+)"
            match = re.search(pattern, v)
            if match:
                # Extract the integer values
                stage_ID = int(match.group(1))
                tran_logicID = int(match.group(2))
                assign_val_str = "assign_stage_" + str(stage_ID) + "_tcam" + str(tran_logicID)
                key_val_str = "key_val_stage_" + str(stage_ID) + "_tcam" + str(tran_logicID)
                key_mask_str = "key_mask_stage_" + str(stage_ID) + "_tcam" + str(tran_logicID)
